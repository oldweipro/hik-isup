package com.oldwei.hikisup.server;
...
public class WebSocketServer {
     ...
     */
    @OnOpen
    public void onOpen(@PathParam("sid") String sid, Session session) throws InterruptedException {
        /**
        ...
        sendToOne(sid, "{\"type\":\"mse\",\"value\":\"video/mp4; codecs=\\\"avc1.4D0029\\\"\"}");
//        sendToOne(sid, "{\"type\":\"mse\",\"value\":\"video/mp4; codecs=\\\"hvc1.1.6.L153.B0\\\"\"}");
    ...
    public static void sendToOne(ByteBuffer buffers) {
        String toSid = "1";
        // Add the message to the queue
        Session toSession = onlineSessionClientMap.get(toSid);
        if (toSession == null) {
            log.error("服务端给客户端发送消息 ==> toSid = {} 不存在, buffers = {}", toSid, buffers);
            return;
        }
        // 异步发送
        try {
            System.out.println(buffers);
            toSession.getBasicRemote().sendBinary(buffers);
        } catch (IOException e) {
            log.error("发送消息失败，WebSocket IO异常");
            e.printStackTrace();
        }
